...
CTEx = ' *yn | *xv | *img '
CTBeforeRowTemp = '^(%s-(?!%s).+?%s)+'
CTBeforeCellTemp = '^(%s(?!%s).+?%s)+'
CTAfterCellTemp = '(%s(?!%s).+?%s)+$'
CTExtraCellTemp = '(%s\+(?!%s).+?%s)+$'

CTBeforeRow = CTBeforeRowTemp % (BLOCK_START_STRING, CTEx, BLOCK_END_STRING)
CTBeforeCell = CTBeforeCellTemp % (BLOCK_START_STRING, CTEx, BLOCK_END_STRING)
CTAfterCell = CTAfterCellTemp % (BLOCK_START_STRING, CTEx, BLOCK_END_STRING)
CTExtraCell = CTExtraCellTemp % (BLOCK_START_STRING, CTEx, BLOCK_END_STRING)
...
def recompile_patterns(user_tags=[]):
    global p_br, p_bc, p_ac, p_ex

    ct_ex = CTEx + '| ' + ' | '.join(['*' + tag for tag in user_tags])

    p_br = re.compile(CTBeforeRowTemp % (BLOCK_START_STRING, ct_ex, BLOCK_END_STRING))
    p_bc = re.compile(CTBeforeCellTemp % (BLOCK_START_STRING, ct_ex, BLOCK_END_STRING))
    p_ac = re.compile(CTAfterCellTemp % (BLOCK_START_STRING, ct_ex, BLOCK_END_STRING))
    p_ex = re.compile(CTExtraCellTemp % (BLOCK_START_STRING, ct_ex, BLOCK_END_STRING))
